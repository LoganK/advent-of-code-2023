...
data class CubeSet(val cubes: Map<String, Int>) {
  companion object {
    fun fromString(input: String): CubeSet =
        CubeSet(input.split(", ").map { it.split(" ") }.map { it[1] to it[0].toInt() }.toMap())
  }

  fun isSubset(arg: CubeSet): Boolean = arg.cubes.all { cubes[it.key] ?: 0 >= it.value }

  fun minBag(rhs: CubeSet): CubeSet =
      CubeSet(
          cubes.keys
              .union(rhs.cubes.keys)
              .map { it to maxOf(cubes.getOrDefault(it, 0), rhs.cubes.getOrDefault(it, 0)) }
              .toMap())
}
...
data class Game(val id: Int, val sets: List<CubeSet>) {
  ...
  fun minBag(): CubeSet = sets.reduce { acc, p -> acc.minBag(p) }
}
